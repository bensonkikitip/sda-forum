...
export function RegionForumsManager({
  regionId,
  allForums,
  enabledForumIds: initial,
}: Props) {
  ...
  async function toggle(forumId: string, on: boolean) {
    ...
    if (on) {
      ...
      if (error) {
        ...
        toast.success('Forum scoped to this region')
        router.refresh()
      }
    } else {
      const { error } = await supabase
      ...
      if (error) {
        ...
        toast.success('Forum unscoped from this region')
        router.refresh()
      }
    }
    setToggling(null)
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base">Region Forums</CardTitle>
        <CardDescription>
          Toggle which forums are scoped to this region. Members in this region auto-subscribe to enabled forums.
        </CardDescription>
      </CardHeader>
